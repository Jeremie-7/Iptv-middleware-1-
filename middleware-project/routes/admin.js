...
router.post("/register", (req, res) => {
  try {
    ...
    // Les packs sont OPTIONNELS à l'inscription — le client les choisit après connexion
    const chosenPacks = Array.isArray(packsArr) && packsArr.length > 0
      ? packsArr
      : (packId && packId !== '' ? [packId] : []);

    // Vérifie les packs seulement s'il y en a
    ...
    const firstPack = chosenPacks.length > 0
      ? (chainesJson.packs || []).find(p => p.id === chosenPacks[0])
      : null;
